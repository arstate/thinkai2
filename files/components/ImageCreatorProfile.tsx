...
interface ImageCreatorProfileProps {
    name: string;
...
const CopyIcon: React.FC<{ className: string }> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={className}>
      ...
      <path d="M5 5.5A1.5 1.5 0 0 0 3.5 7v9A1.5 1.5 0 0 0 5 17.5h5A1.5 1.5 0 0 0 11.5 16V7a1.5 1.5 0 0 0-1.5-1.5h-5Z" />
    </svg>
);
...
const ImageCreatorProfile: React.FC<ImageCreatorProfileProps> = ({ name, profilePicUrl, messages, onClose, onImageClick }) => {
    const [copiedPromptId, setCopiedPromptId] = useState<string | null>(null);

    const imageMessages = messages.filter(msg => msg.sender === MessageSender.AI && msg.imageUrl);
            ...
            <main className="flex-1 overflow-y-auto p-4">
                {imageMessages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 dark:text-gray-400">
                        ...
                        <h2 className="text-xl font-bold">Belum Ada Gambar</h2>
                        <p>Gambar yang kamu buat akan muncul di sini.</p>
                    </div>
                ) : (
                        ...
                        {[...imageMessages].reverse().map((msg) => (
                           <div key={msg.id}>
                                <button
                                    ...
                                    className="aspect-square block w-full bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden group focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-gray-900"
                                    aria-label={`Lihat gambar yang dibuat dengan prompt: ${msg.imagePrompt}`}
                                >
                                    <img
                                        src={msg.imageUrl}
                                        alt={`Generated image ${msg.id}`}
                                        className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110"
                                    />
                                </button>
                                {msg.imagePrompt && (
                                    <div className="mt-2 text-left">
                                        {msg.imagePrompt === 'background removed' ? (
                                            <p className="text-gray-500 dark:text-gray-400 text-sm italic font-semibold">
                                                Background removed
                                            </p>
                                        ) : (
                                            <>
                                                <p className="text-gray-700 dark:text-gray-300 text-xs line-clamp-2" title={msg.imagePrompt}>
                                                    {msg.imagePrompt}
                                                </p>
                                                <button 
                                                    onClick={(e) => handleCopy(e, msg.imagePrompt!, msg.id)}
...
export default ImageCreatorProfile;
